...
load_fragment_to_env() {
  ...
  while IFS= read -r line || [ -n "$line" ]; do
    ...
    val="${line#*=}"
    case "$key" in
      AUTH_OWNER_PASSWORD|AUTH_OWNER_SECURITY_ANSWER|AUTH_SESSION_DAYS|PEXELS_API_KEY|PIXABAY_API_KEY|TENCENT_COS_SECRET_ID|TENCENT_COS_SECRET_KEY|TENCENT_COS_BUCKET|TENCENT_COS_REGION|TENCENT_COS_BASE_URL|OWNER_PUBLISH_GITHUB_TOKEN|OWNER_PUBLISH_GITHUB_OWNER|OWNER_PUBLISH_GITHUB_REPO|OWNER_PUBLISH_GITHUB_BRANCH)
        export "$key"="$val"
        ;;
    esac
  done <"$frag"
