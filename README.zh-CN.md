...
- 正式进入主站的 `/about` 关于我页面：把关于页预览集成进 React 主站，图片统一引用腾讯 COS，并把项目、游戏、工具卡片作为个人站身份的一部分呈现。
- 已部署的 AI QuizCard 项目展示：包含 `/showcase/quizcard.html` 成品页，以及 `/web/` 下带默认题组、访客可直接练习的静态应用。
...
最近的成长记录也补充了 `/moments` 碎语页面迁移、站长控制台碎语发布链路、Leyili 花园友链修正、首页和其他页面的双层滚动修复、AI 固定答案从站长控制台同步到网站助手的链路、UTF-8 安全邮件通知、友链联系邮箱收集、友链连续回复、站长后台邮箱可见能力、数据中心服务器状态监控、AI 生图结果保存到腾讯 COS，以及新上线的 `/about` 关于我页面和 AI QuizCard 项目展示。
...
主站现在也包含一个正式的 `/about` 页面，页面名称为“关于我”。它通过带作用域隔离的 Shadow DOM 容器把关于页预览加载进 React 主站，移除了预览专用文字，图片统一引用腾讯 COS 链接，项目集在不同断点下保持更舒展的网格，常驻游戏卡片直接展示而不再依赖底部横向拉条，技术工具图标也会连续移动而不出现空白。

关于页里的第一个项目是 AI QuizCard。项目卡片和 `/about/projects/quizcard` 都会指向 `/showcase/quizcard.html`，该成品页随主站部署了展示页资源、小程序风格页面预览，以及 `/web/` 下访客可直接玩的静态 QuizCard 应用。静态访客版内置默认题组，因此没有后端数据时也能马上练习；练习报告页在窄屏下让摘要卡片随页面一起滚动，宽屏下才保留 sticky 摘要，避免错题区覆盖主界面。
...
右下角 AI 助手也加入了面向登录用户的生图模式。它会调用后端 `/api/ai/image`，使用 Agnes AI 的 `agnes-image-2.1-flash`，把生成图保存到腾讯 COS，并把最终 COS 链接、提示词和生成时间记录给 `/ai-gallery` 生成图广场，同时在屏幕中央用透明浅色镭射卡片展示结果，支持保存本地和复制最终 COS 链接。
...
| `guestbook_messages.go`, `guestbook_user.go`, `guestbook_ip.go`, `mail_notifier.go` | `GET/POST /api/guestbook/messages`, `PATCH/DELETE /api/guestbook/messages/:id`, `PATCH /api/guestbook/messages/:id/status` | 公开可见/可发布留言；站长/管理员可审核 | 当前留言板和朋友页留言控制器，支持分区、连续回复、访客/登录用户发布、友链联系邮箱必填、UTF-8 邮件通知、限流、防重复、IP 脱敏和软审核。 |
...
| `POST /api/guestbook/messages` | `{ "nickname": "...", "contactEmail": "...", "content": "...", "parentId": 0, "channel": "guestbook" }` | 普通留言墙未登录时提交 `nickname` + `content`，登录后提交 `content` 即可。顶层 `friends` 友链留言必须提供 `nickname` 和 `contactEmail`；公开响应不会暴露 `contactEmail`。回复会忽略提交的 `contactEmail`，并在可用时通知父留言联系人或账号邮箱。 |
...
| `GET /api/ai/image/gallery?limit=60&before=...` | 可选游标查询 | 公开生成图广场 feed，返回 `prompt`、`imageUrl`、`size`、`createdAt` 和匿名安全的 `author` 展示文本。 |
...
- AI 生图：配置 `AGNES_API_KEY`；可选配置 `AGNES_BASE_URL`（默认 `https://apihub.agnes-ai.com`）以及 `AGNES_IMAGE_MODEL` 或 `AI_IMAGE_MODEL`（默认 `agnes-image-2.1-flash`）。如果没有 `AGNES_API_KEY`，后端仍会回退兼容旧的 DashScope 变量 `DASHSCOPE_API_KEY`、`DASHSCOPE_BASE_URL`、`AI_IMAGE_MODEL` 和 `ALIYUN_BAILIAN_API_KEY`。
...
- 继续填充项目集：让关于页里的项目卡片逐步补上真实项目页、截图、体验入口和版本记录。
- 优化 AI QuizCard 导入流程：在默认题组可试玩的基础上，让用户导入、解析、保存题库和进入练习的路径更顺。
