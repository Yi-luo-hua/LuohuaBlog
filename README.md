...
| `POST /api/guestbook/messages` | `{ "nickname": "...", "contactEmail": "...", "content": "...", "parentId": 0, "channel": "guestbook" }` | Requires a valid `acg_session` from email login; anonymous requests return `401 LOGIN_REQUIRED`. Normal guestbook messages only need `content`. Top-level `friends` messages require `nickname` and `contactEmail`; public responses never expose `contactEmail`. Replies ignore submitted `contactEmail` and notify the parent contact/account email when available. |
